...
class MileageController extends Controller
{
    ...
    {
        return view('enterprise.mileage')->with([
    ...
    public function sum(Request $request, $id)
    {
        ...
        $distance = $this->repository->skipPresenter()->all()->sum(function($v) {
            return $v->value;
        });
    ...
    public function report(Request $request, $id)
    {
        ...
        $records = $this->repository->skipPresenter()->all()->map(function($v) {
            return [
                'day' => $v->when->format('j'),
                'val' => round($v->value / 1000, 1),
            ];
        });
    ...
    public function export(Request $request)
    {
     ...
     $mon = $from->format('M');
     $month =$from->format('F');
      Excel::create('MileageReport of '.$month, function ($excel) use ($wholedata, $mon) {
