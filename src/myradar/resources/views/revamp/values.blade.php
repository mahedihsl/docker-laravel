<!-- ======= Values Section ======= -->
@php
$hasHttps= env('APP_ENV')=='production'
@endphp
<section id="values" class="values">

  <div class="container" data-aos="fade-up">

    <header class="section-header">
      <h2>Our Values</h2>
      <p class="bangla">@lang('values.title')</p>
    </header>

    <div class="row">

      <div class="col-lg-4">
        <div class="box" data-aos="fade-up" data-aos-delay="200">
          <img src="{{ asset('landing2/assets/image/live_tracking.png', $hasHttps) }}" class="img-fluid" alt="">
          <h3 class="bangla">@lang('values.item1.title')</h3>
          <p class="bangla">@lang('values.item1.message')</p>
        </div>
      </div>

      <div class="col-lg-4 mt-4 mt-lg-0">
        <div class="box" data-aos="fade-up" data-aos-delay="400">
          <img src="{{ asset('landing2/assets/image/lock_unlock.png', $hasHttps) }}" class="img-fluid" alt="">
          <h3 class="bangla">@lang('values.item2.title')</h3>
          <p class="bangla">@lang('values.item2.message')</p>
        </div>
      </div>

      <div class="col-lg-4 mt-4 mt-lg-0">
        <div class="box" data-aos="fade-up" data-aos-delay="600">
          <img src="{{ asset('landing2/assets/image/cng_monitoring.png', $hasHttps) }}" class="img-fluid" alt="">
          <h3 class="bangla">@lang('values.item3.title')</h3>
          <p class="bangla">@lang('values.item3.message')</p>
        </div>
      </div>

    </div>

  </div>

</section><!-- End Values Section -->
