export default class PaymentApi {
    ...
    findCustomer(phone) {
        Vue.http.get(`/find/customer/${phone}`).then(
            response => {
    ...
    savePayment(data) {
        ...
        Vue.http.post("/save/payment", data).then(
            response => {
                this.EventBus.$emit(
                    "bill-save-finish",
    ...
    paymentList(userId) {
        return new Promise((resolve, reject) => {
            Vue.http.get(`/payment/paymentlist/${userId}`).then(
                response =>
                    response.status ? resolve(response.data) : reject(),
                error => reject()
            );
        });
    }
    ...
    getMsgContent(userId) {
        ...
        Vue.http.get(`/payment/message/${userId}`).then(
            response => {
                this.EventBus.$emit(
    ...
    getTotalDue(userId) {
        Vue.http.get(`/payment/total-due/${userId}`).then(
            response => {
                this.EventBus.$emit(
                    "total-due-received",
                    response.body.total
    ...
    getPaymentMethod(userId) {
        this.EventBus.$emit("message-send-start");
        Vue.http.get(`/payment/method/sms/${userId}`).then(
            response => {
                this.EventBus.$emit(
                    "message-content-received",
                    response.body
    ...
    sendMethodAll() {
        return new Promise(function(resolve, reject) {
            Vue.http.get("/payment/method/sms").then(
                response => {
                    if (response.body.status == 1) {
